...
document.addEventListener("mouseup", () => {
  if (currentDraggedBlock) {
    ...
    for (const block of blocks) {
      ...
      if (distanceRight <= SNAP_DISTANCE && draggedRect.bottom > blockRect.top && draggedRect.top < blockRect.bottom) {
        currentDraggedBlock.style.left = `${block.offsetLeft + block.offsetWidth + SNAP_OFFSETS.right.x}px`;
        currentDraggedBlock.style.top = `${block.offsetTop + SNAP_OFFSETS.right.y}px`;
        snapped = true;
        break;
      }

      if (distanceBottom <= SNAP_DISTANCE && draggedRect.right > blockRect.left && draggedRect.left < blockRect.right) {
        currentDraggedBlock.style.left = `${block.offsetLeft + SNAP_OFFSETS.bottom.x}px`;
        currentDraggedBlock.style.top = `${block.offsetTop + block.offsetHeight + SNAP_OFFSETS.bottom.y}px`;
        snapped = true;
        break;
      }
    }
